...
int Page::read(std::ifstream &is) {
    ...
    for (int j = 0; j < this->capacityInRecords(); j++) {
        // 1.1 read page.capacityInRecords() records
        char *recData = new char[Config::RECORD_SIZE * this->capacityInRecords()];
        is.read(recData, Config::RECORD_SIZE * this->capacityInRecords());
        int nBytesRead = is.gcount();
        ...
        if (nBytesRead % Config::RECORD_SIZE != 0) {
            throw std::runtime_error("Error: Read " + std::to_string(nBytesRead) +
                                     " bytes, expected " + std::to_string(Config::RECORD_SIZE));
        }
